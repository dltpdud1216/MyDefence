...
namespace MyDefence
{
    ...
    public class LevelSelect : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            //컨텐트 길이
            float contectnRectHeight = (1 + (int)(levelButtons.Length / 5)) * (110 + 7);
            //실제 스크롤량
            float scrollHeight = contectnRectHeight - scrollrect.rect.height;

            if(scrollHeight > 0)
            {
                //자동 스크롤량 - 0~4 : 0, 5~9 : 110+7 , 10~14 : (110+7)*2, 15~19 : (110+7)*3 ....
...
파일이 있으면 유저 데이터를 파일에 저장한 데이터로 초기화

4. 게임 세이브시 체크사항
: 레벨 클리어 데이터는 저장된 데이터와 저장할 데이터를 비교한 후 저장한다

PlaterPrefs : 세이브 / 로드
PlaterPrefs.SetInt(KeyName, Value);     KeyName 이름으로 Value 저장
PlaterPrefs.GetInt(KeyName);            KeyName 이름으로 저장된 Value 가져오기

PlaterPrefs.SetFloat(KeyName, Value);     KeyName 이름으로 Value 저장
PlaterPrefs.GetFloat(KeyName);            KeyName 이름으로 저장된 Value 가져오기

PlaterPrefs.SetString(KeyName, Value);     KeyName 이름으로 Value 저장
PlaterPrefs.GetString(KeyName);            KeyName 이름으로 저장된 Value 가져오기

DeletaAll(); //저장된 모든값 삭제
